...
create table public.treatments (
  ...
  skin_type_at_session text not null,
  sun_exposure_check boolean default false,
  sun_exposed_areas jsonb,
  areas_treated jsonb not null,
  spot_size text,
...
create policy "Admins and MDs can view all patients" on public.patients for select using (exists (select 1 from public.users where id = auth.uid() and role in ('admin', 'md')));
