...
-- Small clinic -- no need for per-tech record isolation.
create policy "All staff can view all patients" on public.patients for select using (
  exists (select 1 from public.users where id = auth.uid() and role in ('admin', 'md', 'tech'))
);

create policy "All staff can view all plans" on public.treatment_plans for select using (
  exists (select 1 from public.users where id = auth.uid() and role in ('admin', 'md', 'tech'))
);

create policy "All staff can view all treatments" on public.treatments for select using (
  exists (select 1 from public.users where id = auth.uid() and role in ('admin', 'md', 'tech'))
);
